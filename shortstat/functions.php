...
function SI_getReferers() {
	...
	if ($result = mysql_query($query)) {
		...
		while ($r = mysql_fetch_array($result)) {
			...
			$ul .= "\t<tr><td><a href=\"$r[referer]\" title=\"$resource\">".SI_trimReferer($url['host'])."</a></td><td class=\"last\">$when</td></tr>\n";
			}
		$ul .= "</table>";
		}
	return $ul;
...
function SI_getDomains() {
	...
	if ($result = mysql_query($query)) {
		...
		while ($r = mysql_fetch_array($result)) {
			if ($i < 36) {
				$resource = ($r['resource']=="/")?$SI_display["siteshort"]:$r['resource'];
				$ul .= "\t<tr><td><a href=\"$r[referer]\" title=\"$resource\">$r[domain]</a></td><td class=\"last\">$r[total]</td></tr>\n";
				$i++;
				}
			}
		$ul .= "</table>";
